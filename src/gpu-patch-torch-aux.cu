...
) 
{
  ...
  uint32_t keep = 1;
  if (buffer->aux != NULL && buffer->torch_aux != NULL && (flags & (GPU_PATCH_SHARED | GPU_PATCH_LOCAL)) == 0) {
    ...
    gpu_patch_aux_address_dict *torch_address_dict = (gpu_patch_aux_address_dict *)buffer->torch_aux;
    gpu_patch_analysis_address_t *torch_start_end = torch_address_dict->start_end;
    uint32_t torch_pos = map_prev(torch_start_end, addr, torch_address_dict->size, gpu_patch_analysis_address_comparator());
